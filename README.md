...
6. `ResultPanelModel` が撮影完了ごとにスコアを累積し、`ResultPanelPresenter` が結果表示。ホームへ戻るで `Home` シーンへ。
...
| UI | `UIPresenter`, `UIView`, `HomePresenter`, `ResultPanelPresenter`, チュートリアル／設定系 | 画面遷移と表示制御 |
| オーディオ | `AudioManager` | BGM・シャッター音 |
| ユーティリティ | `StaticMeshColliderBaker` | SkinnedMesh の現在ポーズを MeshCollider に焼き付け |
| 設定（Koki） | `SettingManager`, `SettingView`, `SettingSO` など | 設定 UI／ScriptableObject 連携 |
